...
core/researcher_zero/learn/
  ...
  context_loader.py     # workspace 四个文件读取（basic_info/taxonomy/network/main_challenge）
  planner.py            # 计划生成与计划重写（复用 core/skills/plan/scripts/plan_tool.py）
  reactor.py            # ReAct 子图（think/act/stop）
  summarizer.py         # 子任务总结 + 全局总结
  graph.py              # 主图拼装（含子图调用）
  design.md
```
...
辅助模型：
- `PlanItem`：`id`、`title`、`status(todo|done)`
...
- 组装 `system_prompt`（持久保存到 state）：
  ...
  - 执行规则（先做任务拆解、按子任务循环 react、每子任务结束必须总结）
...
- `react_think` 的输入消息顺序固定为：
  ...
  3. `HumanMessage(history)`：`condensed_messages` 的压缩历史
...
- `planner.py / reactor.py / summarizer.py` 在节点内根据 state 拼装消息：
  - `planner.py`：`SystemMessage(plan_system_prompt) + HumanMessage(task)`
  - `reactor.py`：`SystemMessage(system_prompt) + HumanMessage(plan) + HumanMessage(history) + HumanMessage(current_subtask)`
  - `summarizer.py`：`SystemMessage(system_prompt) + HumanMessage(summary_instruction + react_trace)`
...
2. `build_plan_context`：加载四类上下文
3. `plan_task`：将输入 task 拆解为初始 `plan_items`（任务拆解，不是动作编排）
4. `select_next_subtask`：选择待执行子任务，若无则转最终总结
...
- `planner.py / reactor.py / summarizer.py` 在调用 LLM 前，按 state 动态构造 `List[Message]`。
...
- 工具注册表建议独立维护，保证 act 节点只做调度与状态更新。
- 任何工具异常直接抛出，不做静默兜底。

## 8.1 Planner 复用策略（强约束）
- `planner.py` 不单独实现计划解析/编辑逻辑。
- 统一复用 `core/skills/plan/scripts/plan_tool.py`：
  - `parse_plan(text)`：解析 LLM 生成的 `<PLAN>...</PLAN>` 为步骤列表
  - `apply_patch(plan, patch_text)`：在执行过程中按 SEARCH/REPLACE 安全改写 plan
- LLM 在 plan/replan 阶段只负责生成文本；结构化解析与计划变更一致性由 `plan_tool.py` 保证。
...
## 11. 最小可落地顺序
1. 先完成 `context_loader + state + prompts + plan_task`（拿到可执行计划）。
2. 再接入 `reactor` 子图（think/act/stop）。
